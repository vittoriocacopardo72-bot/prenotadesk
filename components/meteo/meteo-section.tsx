    ...
    <>
        ...
        <CardHeader className="gap-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <CardDescription>
                Board operativo manuale per decisioni giornaliere su partenze e
                sicurezza
              </CardDescription>
            </div>
          ...
          <div className="grid gap-2 text-xs text-slate-600 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-md bg-slate-50 px-2.5 py-2">
              Sintesi: condizioni generalmente favorevoli con attenzione nel
              pomeriggio.
            </div>
            <div className="rounded-md bg-slate-50 px-2.5 py-2">
              Ultimo aggiornamento manuale: 07:40
            </div>
            <div className="rounded-md bg-slate-50 px-2.5 py-2">
              Responsabile aggiornamento: Marco D.
            </div>
            <div className="rounded-md bg-slate-50 px-2.5 py-2">
              Prossima revisione: 13:00
            </div>
          </div>
        </CardHeader>
      </Card>
      ...
      <div className="grid gap-6 sm:col-span-2 xl:col-span-4 xl:grid-cols-[1.7fr_0.9fr]">
        <Card className="bg-white">
          <CardHeader>
            ...
            <CardDescription>
              Valutazione manuale del rischio operativo per fascia
            </CardDescription>
          </CardHeader>
                ...
                {meteoTimeSlots.map((slot) => (
                  <tr key={slot.fascia} className="border-b border-slate-100">
                    ...
                    <td className="px-2 py-2">
                      <Badge
                        variant={
                          slot.decisione === "Operativo"
                            ? "default"
                            : slot.decisione === "Attenzione"
                              ? "secondary"
                              : "outline"
                        }
                      >
          ...
          <Card className="bg-white">
            ...
            <CardContent className="space-y-3">
              <div className="min-h-24 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                Verificare briefing sicurezza per gruppi oltre 8 ospiti.
                Confermare equipaggiamento anti-spruzzo su barche open nel
                pomeriggio.
              </div>
      ...
      <Card className="bg-white sm:col-span-2 xl:col-span-4">
        <CardHeader>
          ...
          <CardDescription>
            Stato operativo delle prossime uscite
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          {meteoDeparturesImpact.map((item) => (
            <div
              key={`${item.ora}-${item.corsa}`}
