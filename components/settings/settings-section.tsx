    ...
    <>
      <Card className="bg-white sm:col-span-2 xl:col-span-4">
        <CardHeader>
          ...
          <CardDescription>
            Centro di controllo operativo per configurazioni aziendali,
      ...
      <div className="grid gap-6 sm:col-span-2 xl:col-span-4">
        <Card className="bg-white">
          <CardHeader>
            ...
            <CardDescription>
              Separa impostazioni base per utenti standard e impostazioni
              avanzate tecniche
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              <Button
                ...
                onClick={() =>
                  void updateSettingsPreferences({ settingsArea: "Base" })
                }
              >
                Base
              </Button>
              <Button
                ...
                onClick={() =>
                  void updateSettingsPreferences({ settingsArea: "Avanzate" })
                }
              >
                Avanzate
              </Button>
            ...
            {settingsArea === "Avanzate" ? (
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
                Modifica queste impostazioni solo se conosci l&apos;impatto
                operativo.
              </div>
            ) : (
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                Area consigliata per utenti operativi e amministrativi non
                tecnici.
              </div>
            )}
            {groups.map((group) => (
                  ...
                  {group.id === "piani-abbonamento" ? (
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                          Dati dimostrativi: limiti e contatori non collegati a
                          dati reali
                        </Badge>
                      </div>
                        ...
                        {planCards.map((plan) => (
                          ...
                          >
                            ...
                            <Button
                              type="button"
                              variant={
                                plan.nome === "Free" ? "outline" : "default"
                              }
                              size="sm"
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <div className="rounded-lg border border-slate-200 bg-white p-4">
                          ...
                          <p className="mt-1 text-sm font-medium text-slate-800">
                            1/1 utenti attivi, 42/100 prenotazioni mese
                          </p>
                        </div>
                        <div className="rounded-lg border border-slate-200 bg-white p-4">
                          <p className="text-xs text-slate-500">
                            Storico fatture mock
                          </p>
                          <p className="mt-1 text-sm font-medium text-slate-800">
                            Nessuna fattura emessa (profilo Free)
                          </p>
                        </div>
                      </div>
                      <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
                        Gateway di pagamento abbonamento non ancora attivo in
                        questa versione
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {group.id === "preferenze-app" ? (
                        ...
                        >
                          Parzialmente dimostrativo: le righe con
                          «Attiva»/«Disattiva» salvano preferenze sul
                          dispositivo; le altre voci sono testo statico di
                          esempio
                        </Badge>
                      ) : (
                        <Badge
                          ...
                          className="w-fit text-[11px] font-normal text-slate-600"
                        >
                          Dati dimostrativi: configurazione di esempio, non
                          collegata a dati reali o backend
                        </Badge>
                      )}
