...
export function MobileHomeView({
  ...
  const ospitiOggi = todayRows.reduce(
    (s, row) => s + (Number(row.ospiti) || 0),
    0
  );
  ...
  const departuresPreview =
    d0 && d1
      ? `${d0.ora} ${d0.barca} · ${d1.ora} ${d1.barca}`
      : d0
        ? `${d0.ora} ${d0.barca}`
        : "Nessuna partenza";
  ...
  const fleetPreview =
    f0 && f1
      ? `${f0.nome} ${f0.stato.toLowerCase()} · ${f1.nome}`
      : f0
        ? `${f0.nome} · ${f0.stato}`
        : "Flotta";
  ...
  return (
    <div className="flex flex-col gap-2">
      <CockpitStatusBar
        tone={cockpitTone(activeAlerts.map((a) => a.text))}
        line={statusLine}
      />
      ...
      <MobileMarineSheet
        ...
        rows={[
          ...
          {
            label: "Visibilita",
            value: weather.visibilita,
            detail: "Sintesi operativa",
          },
          {
            label: "Stato porto",
            value: weather.statoPorto,
            detail: "Sintesi operativa",
          },
        ]}
