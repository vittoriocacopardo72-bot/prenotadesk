  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge className={cn("shrink-0 border-0 text-[10px] font-semibold", toneBadge[tone])}>
          {tone === "operativo" ? "Operativo" : tone === "attenzione" ? "Attenzione" : "Critico"}
        </Badge>
      </div>
      <p className="text-sm leading-snug font-medium text-slate-900">{metricsLine}</p>
    </div>
  )
}
