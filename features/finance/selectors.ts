...
export function selectFinanceTotals(movements: readonly FinanceMovement[]): FinanceTotals {
  let totaleEntrate = 0
  let totaleUscite = 0
...
export function selectFinanceTodayCount(movements: readonly FinanceMovement[], todayIsoDate: string): number {
  let count = 0
  for (const movement of movements) {
...
export function selectFinanceCategoryShare(movements: readonly FinanceMovement[]): FinanceCategoryShareItem[] {
...
export function selectFinanceSummary(
  ...
  const todayIsoDate = options?.todayIsoDate ?? isoDateFromNow(options?.now ?? new Date())
