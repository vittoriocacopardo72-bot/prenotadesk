...
export function BookingsSection() {
  ...
  const desktopBookingFocusNonce =
    desktopBookingFocus?.bookingFormFocusNonce ?? 0;
  ...
  const summaryStats = useMemo(() => {
    ...
    return {
      ...
      ospitiOggi: todayRows.reduce(
        (s, row) => s + (Number(row.ospiti) || 0),
        0
      ),
      daConfermare: bookingRows.filter((row) => row.stato === "In attesa")
        .length,
    };
  }, [bookingRows]);
