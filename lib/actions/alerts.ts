...
import { getAppState, selectActiveAlerts, updateAppState } from "@/lib/store/app-store"
import type { ActionResult } from "@/types/actions"

export async function markAlertResolved(alertId: string): Promise<ActionResult<{ alertId: string }>> {
  const target = getAppState().alerts.find((a) => a.id === alertId)
  if (!target) {
    return { status: "error", code: ACTION_ERROR.notFound, message: "Alert non trovato." }
  }
